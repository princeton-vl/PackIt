...
class UnityEnvironment(object):
    def __init__(self, file_name=None, worker_id=0,
                 ...
                 pack_file_name="test", pack_id=0, get_gt=False,
                 rot_before_mov=True):
        """
        ...
        :string pack_file_name: The file name containing information about the packs
        :string pack_id: The serial number of pack in pack_file_name to launch
        :bool get_gt: Whether to get the grounftruth actions or not
        :bool rot_before_mov: Whether to add rotation before movement or not
        """
    ...
    def executable_launcher(self, file_name, docker_training, no_graphics,
                            pack_file_name, pack_id, get_gt, rot_before_mov):
        cwd = os.getcwd()
        ...
        if rot_before_mov:
            _rot_before_mov = "true"
        else:
            _rot_before_mov ="false"
        ...
        if launch_string is None:
            ...
            if not docker_training:
			 ...
			 '-logFile',  str(_path) + '/log/unity/' + str(self.port),
                         '--port', str(self.port), '-fileName', pack_file_name,
                         '-packID', str(pack_id), '-getGT', _get_gt,
                         '-rotBeforeMov', _rot_before_mov])
                else:
                    self.proc1 = subprocess.Popen(
                        [launch_string, '--port', str(self.port),
                         '-fileName', pack_file_name, '-packID', str(pack_id),
                         '-getGT', _get_gt, '-rotBeforeMov', _rot_before_mov])
            else:
                """
